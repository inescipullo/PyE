...
proceso <- function(k) { # k es los de exitos esperados
  p <- 0.4 # probabilidad de exito, p < 0.5
  EXITO <- 1 
  FRACASO <- 0
  EM <- c(EXITO,FRACASO)
...
boxplot(trayectorias, col = "yellow", main = "Cantidad de sucesos hasta obtener 3 éxitos consecutivos")
...
# b)
S_n = cumsum(D_n)
plot(1:pasos, S_n, type = "p", xlab = "Pasos", ylab = "Posición de la particula")
...
s = 20 # monto en el que el jugador decide dejar de apostar
evolucion = c(k)
...
plot(0:(length(evolucion)-1), evolucion, xaxt = 'n', type = "p", xlab = "Número de juego", ylab = "Fortuna del jugador")
...
{
s = 20 # monto en el que el jugador decide dejar de apostar
p = 0.5 # probabilidad de ganar una apuesta
x = c(0,1)
probabilities = c(1-p,p)
n = 10000
contador = 0
...
barplot(tabla_trayectorias, ylim = c(0,3500), xlab = "Cantidad de comparaciones (M_5)", ylab = "Cantidad de simulaciones", col = "lightblue")
...
# Apartado b
# Calculo del promedio (esperanza real) de comparaciones a realizar por una lista de 5 elementos
X <- 6:10
P <- c(1/3,4/15,1/5,1/15,2/15)
E <- sum(X*P)
...
barplot(table(cien_pasos), names.arg = c('a','b','c','d','e','f','g'), ylim = c(0,35),xlab = "Página web", ylab = "Cantidad de veces que el usuario paso por la página", col = "lightblue", main = "Frecuencia absoluta de visitas a cada página\n web en un recorrido de 100 pasos.")
...
# esa variable.
apartado_b <- function () {
  t <- 1000 # Simulacion hasta un T suficientemente grande
  ...
  mtext("Intervalos (tiempos entre arribos en horas)", side=1, line=1.5, font=2, cex=1)
